...
Visualizes how many scribe notes were generated each day over the selected time range.
...
A log of recently generated notes, including:
...
### Export Admin Panel Data

**Full CSV Export:** Admins can now download the entire dataset for a selected period rather than just the currently visible paginated page.

- The "Current View" button transforms into a loading icon during the generation of large files. 
...
<Frame>
  ![](/images/admin_panel_user_management.png)
</Frame>
...
<Note>
  Adding a new user does not automatically configure them with the correct features or upgrade them to have unlimited scribes - doing so requires a representative from Commure.
</Note>

**********_To deactivate users, you can toggle a user to a deactivated state via the User Management tab in the Admin Panel._**********
...
**Templates Section:** Displays all templates available to the user, with options to:

- **Add Template**
- Set **Custom Formatting**
- View **Rules Set** if predefined logic is applied

**Recent Scribes:** This section shows a log of recently generated notes by the user.
...
<Frame>
  ![](/images/admin_panel_template_library.png)
</Frame>
...
Please reach out to [support@ambient.commure.com](mailto:support@ambient.commure.com) to expand your liscence account, or to enable features or unlimited scribes on a newly created account.

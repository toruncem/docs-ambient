...
- **Non-Integrated Sites**: Selecting **Carry Forward **from the**AI Actions ** menu of the patient's previous note, opens the new recording page for the current visit of that same pateint, with the previous note loaded as context. The system uses this to generate the new note once a new recording is completed.

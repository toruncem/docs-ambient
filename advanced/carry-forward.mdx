...
- **EHR-Integrated Sites**: Once configured, carry forward will appear for eligible notes. The new note is automatically created using content from the previous visit. You’ll be taken directly to the editing screen with carried-forward data already populated.
- **Non-Integrated Sites**: Selecting **Carry Forward from the AI Actions**  menu of the patient's previous note, opens the new recording page for the current visit of that same patient, with the previous note loaded as context. The system uses this to generate the new note once a new recording is generated. Additionally, users are able to pick previous scribes from the visits page and apply carry forward directly to them.

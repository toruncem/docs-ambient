...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST https://api.scribe.athelas.com/v1/ambient/api/auth/token \
  ...
  <Tab title="JavaScript">
    ...
    const res = await fetch('https://api.scribe.athelas.com/v1/ambient/api/auth/token', {
      method: 'POST',
      headers: {
  ...
  <Tab title="Python">
    ...
    response = requests.post(
        'https://api.scribe.athelas.com/v1/ambient/api/auth/token',
        data={
...
```bash
curl -X GET https://api.scribe.athelas.com/v1/ambient/api/scribe/12345 \
  -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'
```

## Token Expiration

Tokens have a default expiration time of 172,800 seconds (48 hours). The `expires_in` field in the response indicates when the token will expire.
